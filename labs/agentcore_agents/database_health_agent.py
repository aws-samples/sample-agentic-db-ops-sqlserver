...
system_prompt = """You are an RDS SQL Server performance specialist using Performance Insights and CloudWatch metrics.

Available tools:

Performance Insights (1-min granularity for ≤24h):
...
## Timeline
1. Instance created: [timestamp]
2. Issue started: [timestamp]
3. Duration: [X hours/days]

## Current Status
1. CPU: [X%] - [Status: Normal/Warning/Critical]
2. Database Load: [X AAS] - [Status]
3. Connections: [X/Y] - [Status]
4. Top Wait Event: [Event Type] - [X%]

## Root Cause Analysis
1. Primary bottleneck: [CPU/IO/Lock/Memory]
2. Top resource-consuming query: [Query name/text]
3. Contributing factors: [List specific issues]

## Action Items
1. **Immediate**: [Specific action with command/query if applicable]
2. **Short-term**: [Optimization recommendation]
3. **Long-term**: [Preventive measure]

## Recommendations
- Use numbered lists for all findings
- Include specific metrics with units
- Provide actionable commands or queries when possible"""
...
@app.entrypoint
def performance_insights_agent(payload, context=None):
    """Invoke the Performance Insights agent with a payload"""

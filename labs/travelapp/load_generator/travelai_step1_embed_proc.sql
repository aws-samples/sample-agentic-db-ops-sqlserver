...
BEGIN
    ...
    DECLARE @payload NVARCHAR(MAX) = N'{"inputText":"' + STRING_ESCAPE(@text, 'json') + N'","dimensions":1024,"normalize":true}';
    ...
    SET @vector = CAST(JSON_QUERY(@response, '$.result.embedding') AS VECTOR(1024));
END;
GO

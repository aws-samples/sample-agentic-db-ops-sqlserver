...
PRINT '  ✅ Created destinations: ' + CAST((SELECT COUNT(*) FROM Destinations) AS VARCHAR);
...
PRINT '  ✅ Created users: ' + CAST((SELECT COUNT(*) FROM Users) AS VARCHAR);
...
DECLARE @maxTagID INT = (SELECT MAX(TagID) FROM PreferenceTags);
SET @batch = 1;
...
BEGIN
    ...
    SELECT
        ...
        CAST((ABS(CHECKSUM(NEWID())) % 100) / 100.0 AS DECIMAL(3,2))
    FROM UserBatch u
    CROSS APPLY (SELECT TOP (3 + ABS(CHECKSUM(NEWID())) % 6) n = 1 FROM sys.all_objects) tags
    WHERE NOT EXISTS (
        SELECT 1 FROM UserPreferences up
        WHERE up.UserID = u.UserID AND up.TagID = (ABS(CHECKSUM(NEWID())) % @maxTagID) + 1
    );
...
PRINT '  ✅ Created user preferences: ' + CAST((SELECT COUNT(*) FROM UserPreferences) AS VARCHAR);
...
PRINT '  ✅ Created activities: ' + CAST((SELECT COUNT(*) FROM Activities) AS VARCHAR);
...
PRINT '  ✅ Created activity tags: ' + CAST((SELECT COUNT(*) FROM ActivityTags) AS VARCHAR);
...
PRINT '  ✅ Created destination-activity links: ' + CAST((SELECT COUNT(*) FROM DestinationActivities) AS VARCHAR);
...
PRINT '  ✅ Created hotels: ' + CAST((SELECT COUNT(*) FROM Hotels) AS VARCHAR);
...
PRINT '  ✅ Created hotel amenities: ' + CAST((SELECT COUNT(*) FROM HotelAmenities) AS VARCHAR);
...
PRINT '  ✅ Created flights: ' + CAST((SELECT COUNT(*) FROM Flights) AS VARCHAR);
...
PRINT '  ✅ Created bookings: ' + CAST((SELECT COUNT(*) FROM Bookings) AS VARCHAR);
...
SELECT TOP 200000
    CASE ABS(CHECKSUM(NEWID())) % 2 WHEN 0 THEN 'Hotel' ELSE 'Flight' END,
    CASE ABS(CHECKSUM(NEWID())) % 2
        WHEN 0 THEN (ABS(CHECKSUM(NEWID())) % (SELECT COUNT(*) FROM Hotels)) + 1
        ELSE (ABS(CHECKSUM(NEWID())) % (SELECT COUNT(*) FROM Flights)) + 1
    END,
    DATEADD(DAY, n % 365, GETDATE()),
    10 + ABS(CHECKSUM(NEWID())) % 90,
    ABS(CHECKSUM(NEWID())) % 50
FROM (SELECT TOP 200000 ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS n FROM sys.all_objects a CROSS JOIN sys.all_objects b) nums;

PRINT '  ✅ Created availability records: ' + CAST((SELECT COUNT(*) FROM AvailabilityInventory) AS VARCHAR);
...
PRINT '  ✅ Created reviews: ' + CAST((SELECT COUNT(*) FROM Reviews) AS VARCHAR);
